...
class Parser():
    ...
    def __init__(self, txt, mode='Normal'): # TODO: уточнить regex для выздоровевших
        self.regexes = {'russia_new_cases' : r'случа\w+ коронавирусной инфекции COVID\W19\s?–\s(\d+\s?\d+) в \d+ рег|случа\w+ новой коронавирусной инфекции COVID\W19\s?–\s(\d+\s?\d+) в \d+ рег|случа\w+ новой коронавирусной инфекции \(COVID\W19\)\s?–\s(\d+\s?\d+) в \d+ рег',
           ...
           'russia_new_deaths' : r'За последние сутки умер\w+ (\d+) челове|Умерл\w за последние сутки (\d+) челове|За последние сутки подтвержден\w? (\d+) летальн\w+ случа',
           'russia_new_recovered' : r'За последние сутки выписан\w? (\d+\s\d+)\s+челов|За прошедшие сутки выписан\w? по выздоровлени\w\s{1,3}(\d+\s?\d+) человек',
           ...
           'moscow_new_deaths' : r'За последние сутки умер\w* \d+ человек.*Москва\s+\W?\s?(\d+).*Умер\w* за весь период|Умер\w* за последние сутки \d+ человек.*Москва\s+\W?\s?(\d+).*За весь период умерл|Умер\w* за последние сутки \d+ человек.*Москва\s+\W?\s?(\d+).*Умер\w* за весь период|За последние сутки умер\w* \d+ человек.*Москва\s+\W?\s?(\d+).*За весь период умерл|За последние сутки подтвержден\w+ \d+ летальн\w+ случа.*Москва\s+\W?\s?(\d+).*За весь период по России умер',
           'moscow_new_recovered' : r'За последние сутки выписан\w*.*человек\w?.*Москва\s+\W?\s?(\d{3,6}).*За весь период выписан|За последние сутки выписан\w*.*человек\w?.*Москва\s+\W?\s?(\d{3,6}).*Выписан\w* за весь|За прошедшие сутки выписан\w? по выздоровлени\w\s{1,2}\d+\s?\d+ человек.*Москва\s+\W?\s?(\d{3,6}).*За весь период выписано',
           'golden_cite' : r'(За последние сутки в.*без клинических проявлений)',
           'russia_total_deaths' : r'За весь период умер\w+\s+(\d+\s?\d+)\s+челов|Умер\w* за весь период (\d+\s?\d+) челов|За весь период по России умер\w+ (\d+\s?\d+) человек|За весь период по России умер\w? (\d+\s?\d+) чел|За весь период по России умерл\w (\d+\s?\d+)\s\s?человек',
           'russia_total_recovered' : r'За весь период выписан\w+\s+(\d+\s?\d+\s?\d+)|За весь период выписано по выздоровлению по России\s+–?\s?(\d+\s?\d+\s?\d+)'
          }
