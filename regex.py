...
regexes = {
          ...
          r'За последние сутки подтвержден\w? {1,3}(\d+) {1,3}летальн\w+ случа',
          r'За последние сутки подтвержден\w? {1,3}(\d+) {1,3}смерт'
          )),
          ...
          '|'.join((
          r'За последние сутки умер\w* \d+ человек.*Москва\s+\W?\s?(\d+).*Умер\w* за весь период',
          r'Умер\w* за последние сутки \d+ человек.*Москва\s+\W?\s?(\d+).*За весь период умерл',
          r'Умер\w* за последние сутки \d+ человек.*Москва\s+\W?\s?(\d+).*Умер\w* за весь период',
          r'За последние сутки умер\w* \d+ человек.*Москва\s+\W?\s?(\d+).*За весь период умерл',
          r'.*'.join((
                    r'За последние сутки подтвержден\w+ {1,3}\d+ {1,3}летальн\w+ случа',
                    r'Москва\s+\W?\s?(\d+)',
          ...
          r'За весь период по России умерл\w {1,3}(\d+\s?\d+) {1,3}человек'
          )),
